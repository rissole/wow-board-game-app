    ...
    return(
        <div style={{width: '100%', backgroundColor: '#fff'}}>
            ...
            <div style={{display: 'flex', justifyContent: 'center'}}>
                {props.values.map(value => (<ValueEdit {...value}/>))}
            </div>
        </div>
    )
}
...
const ValueEdit = (props: ValueProps) => {
    ...
    return (
        <ValueEditContainer>
            <ValueEditButton disabled={props.value >= MAX_ALLOWED_VALUE} onClick={increaseValue}>+</ValueEditButton>
            <div style={{textAlign: 'center'}}>
                {props.value}
            </div>
            <ValueEditButton disabled={props.value <= MIN_ALLOWED_VALUE} onClick={decreaseValue}>-</ValueEditButton>
        </ValueEditContainer>
    )
}
